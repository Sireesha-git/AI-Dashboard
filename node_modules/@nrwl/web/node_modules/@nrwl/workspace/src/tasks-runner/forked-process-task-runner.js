...
class ForkedProcessTaskRunner {
    ...
    forkProcessForBatch({ executorName, taskGraph }) {
        return new Promise((res, rej) => {
            try {
                const env = this.envForForkedProcess(taskGraph.tasks[0].target.target, process.env.FORCE_COLOR === undefined
                    ? 'true'
    ...
    envForForkedProcess(target, forceColor, outputPath, forwardOutput) {
        ...
        const env = Object.assign(Object.assign(Object.assign(Object.assign({}, envsFromFiles), { FORCE_COLOR: forceColor }), process.env), { NX_INVOKED_BY_RUNNER: 'true', NX_WORKSPACE_ROOT: this.workspaceRoot });
        if (outputPath) {
    ...
    envForForkedProcessForTask(task, forceColor, outputPath, forwardOutput) {
        let env = this.envForForkedProcess(task.target.target, forceColor, outputPath, forwardOutput);
        ...
        env = Object.assign(Object.assign(Object.assign({}, env), envsFromFiles), { NX_TASK_TARGET_PROJECT: task.target.project, NX_TASK_HASH: task.hash });
