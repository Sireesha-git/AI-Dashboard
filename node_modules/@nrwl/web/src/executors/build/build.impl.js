...
function run(options, context) {
    return tslib_1.__asyncGenerator(this, arguments, function* run_1() {
        ...
        if (!options.buildLibsFromSource && context.targetName) {
            const projGraph = yield tslib_1.__await(project_graph_1.createProjectGraphAsync());
        ...
        return yield tslib_1.__await(yield tslib_1.__await(yield* tslib_1.__asyncDelegator(tslib_1.__asyncValues(rxjs_for_await_1.eachValueFrom(rxjs_1.from(configs).pipe(
        // Run build sequentially and bail when first one fails.
        operators_1.mergeScan((acc, config) => {
            if (!acc.hasErrors()) {
                return run_webpack_1.runWebpack(config, webpack.webpack).pipe(operators_1.tap((stats) => {
